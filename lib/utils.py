...
def sample_points_from_mesh(path, n_pts, with_normal=False, fps=False, ratio=2):
    ...
    """
    vertices, faces = load_obj(path)  #3888，3    1946，3
    if fps:
...
def load_depth(img_path):
    ...
    depth = cv2.imread(depth_path, -1) #包括alpha参数
    if len(depth.shape) == 3:
        # This is encoded depth image, let's convert
        # NOTE: RGB is actually BGR in opencv
        depth16 = depth[:, :, 1]*256 + depth[:, :, 2]  # 0-255
...
def compute_mAP(pred_results, out_dir, degree_thresholds=[180], shift_thresholds=[100],
    ...
    iou_aps = np.zeros((num_classes + 1, num_iou_thres))    #每个category的IoU列表
...
def plot_mAP_mean_real(iou_aps, pose_aps, out_dir, iou_thres_list, degree_thres_list, shift_thres_list):
    """ Draw iou 3d AP vs. iou thresholds.
    """
    plt.style.use('bmh') 
    plt.rc('font',family='Times New Roman')
    labels = ['6D-ViT(Ours)', 'SPD(ECCV2020)', 'NOCS(CVPR2019)', 'NOF(ECCV2020)']
    colors = ['tab:red', 'tab:green', 'tab:pink', 'tab:purple', 'tab:gray']
    styles = ['-', 'dotted', 'dashed', 'dashdot']

    fig, (ax_iou, ax_degree, ax_shift) = plt.subplots(1, 3, figsize=(12, 3.5))
    # IoU subplot
    ...
    ax_degree.set_ylabel('Average Precision')
    ...
    ax_shift.set_ylabel('Average Precision')
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(os.path.join(out_dir, 'mAP_mean_real_test.pdf'), format='pdf')
    plt.close(fig)
    return
...
def draw_detections(img, out_dir, data_name, img_id, intrinsics, pred_sRT, pred_size, pred_class_ids,
    ...
    if draw_gt:
        for i in range(gt_sRT.shape[0]):
            if gt_class_ids[i] in [1, 2, 4]:  #index start from 1.
