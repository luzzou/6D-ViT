...
from lib.network_PIF_POF import DeformNet
...
parser.add_argument('--model', type=str, default='model_real.pth', help='resume from saved model')
...
if opt.data == 'val':
    result_dir = 'eval_camera'
    file_path = 'CAMERA/val_list.txt'
    cam_fx, cam_fy, cam_cx, cam_cy = 577.5, 577.5, 319.5, 239.5
else:
    result_dir = 'eval_real'
...
def detect():
    ...
    estimator = torch.nn.DataParallel(estimator, device_ids=[0])  # multi-gpu training

    estimator.cuda()

    parameters = filter(lambda p: p.requires_grad, estimator.parameters())
    ...
    for path in tqdm(img_list):
        ...
        mrcnn_path = os.path.join('deformnet_eval/mrcnn_results', opt.data, 'results_{}_{}_{}.pkl'.format(
        ...
        for i in range(num_insts):
            ...
            # because of how we compute the bbox in function get_bbox
            # there might be a chance that no foreground points after cropping the mask
            # caused by false positive of mask_rcnn, most of the regions are background
            if len(choose) < 32:
...
def evaluate():
    ...
    # messages.append('3D IoU at 25: {:.2f}'.format(iou_aps[1, iou_25_idx] ))
    ...
    # messages.append('3D IoU at 25: {:.2f}'.format(iou_aps[2, iou_25_idx] * 100))
    ...
    # messages.append('3D IoU at 25: {:.2f}'.format(iou_aps[3, iou_25_idx] * 100))
    ...
    messages.append('can:')
    # messages.append('3D IoU at 25: {:.2f}'.format(iou_aps[4, iou_25_idx] * 100))
    ...
    # messages.append('3D IoU at 25: {:.2f}'.format(iou_aps[5, iou_25_idx]))
    ...
    messages.append('mug:')
    # messages.append('3D IoU at 25: {:.2f}'.format(iou_aps[6, iou_25_idx] * 100))
    ...
    # load spd results
    pkl_spd_path = os.path.join('results/deformnet_results',
                                opt.data, 'mAP_Acc.pkl')
    ...
    iou_aps = np.concatenate((iou_aps, spd_iou_aps[None, :], nocs_iou_aps[None, :]), axis=0)
    pose_aps = np.concatenate((pose_aps, spd_pose_aps[None, :, :], nocs_pose_aps[None, :, :]), axis=0)
    # plot
