...
def detect():
    ...
    estimator = torch.nn.DataParallel(estimator)  # multi-gpu training
    estimator.cuda()
    ...
    for path in tqdm(img_list):
        ...
        mrcnn_path = os.path.join('/home/lz/Desktop/6D-ViT/results/mrcnn_results', opt.data, 'results_{}_{}_{}.pkl'.format(
        ...
        for i in range(num_insts):
            ...
            points = np.concatenate((pt0, pt1, pt2), axis=1)  #observed point cloud
            rgb = raw_rgb[rmin:rmax, cmin:cmax, :]

            rgb = cv2.resize(rgb, (opt.img_size, opt.img_size), interpolation=cv2.INTER_LINEAR)
            rgb = norm_color(rgb)

            crop_w = rmax - rmin  #200
            ratio = opt.img_size / crop_w  #0.96
        ...
        if len(valid_inst):
            ...
            t_now = time.time()
            assign_mat, deltas = estimator(f_points, f_rgb, f_choose, f_catId, f_prior)  #num_instance,1024,1024  num,1024,3

            # assign_mat, deltas = estimator(f_rgb, f_choose, f_catId, f_prior)
            inst_shape = f_prior + deltas  #num, 1024, 3

            assign_mat = F.softmax(assign_mat, dim=2)
            f_coords = torch.bmm(assign_mat, inst_shape)  # num x n_pts x 3
            ...
            for i in range(len(valid_inst)):
                ...
                nocs_coords = f_coords[i, choose, :]  #1024,3

                f_size[inst_idx] = 2 * np.amax(np.abs(f_insts[i]), axis=0) #instance size
                points = f_points[i, choose, :]  #observed
